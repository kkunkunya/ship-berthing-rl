...
from src.train.train_vectorized import DEVICE
from src.utils.disturbance_metrics import override_curriculum_disturbance_config


parser = argparse.ArgumentParser(description="Evaluate PPO/TD3 baselines")
parser.add_argument("--algorithm", type=str, required=True, choices=["ppo", "td3"])
parser.add_argument("--model_path", type=str, required=True)
parser.add_argument("--episodes", type=int, default=100)
parser.add_argument("--output", type=str, default="assets/results/baselines/baseline_evaluation.json")
...
model_cls = PPO if args.algorithm == "ppo" else TD3
...
if use_vectorized:
    ...
    while total_episodes < args.episodes:
        ...
        if action_np.ndim == 1:
            action_np = action_np.reshape(args.num_envs, -1)
        action = torch.as_tensor(action_np, device=env.device, dtype=obs.dtype)
